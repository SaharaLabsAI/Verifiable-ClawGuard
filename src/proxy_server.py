...
def _extract_tool_arguments_from_openai_response(response_data: Dict) -> List[str]:
    """Extract tool/function arguments generated by the assistant for guardrail checks."""
    if not isinstance(response_data, dict):
        return []
    ...
    if isinstance(tool_calls, list):
        for call in tool_calls:
            ...
            if isinstance(arguments, str) and arguments.strip():
                extracted.append(arguments.strip())
            elif isinstance(arguments, dict):
                extracted.append(json.dumps(arguments, separators=(",", ":")))
...
def _build_moderation_input_text(assistant_text: str, tool_arguments: List[str]) -> str:
    """Combine assistant text and tool call params for moderation checks."""
    parts: List[str] = []

    assistant_clean = assistant_text.strip() if isinstance(assistant_text, str) else ""
    if assistant_clean:
        parts.append(assistant_clean)

    if isinstance(tool_arguments, list):
        for idx, argument in enumerate(tool_arguments, start=1):
            if isinstance(argument, str) and argument.strip():
                parts.append(f"[tool_call_param_{idx}] {argument.strip()}")

    return "\n\n".join(parts)


def _append_warning_to_text(text: str, warning: str) -> str:
    """Append warning once to plain text content."""
    if warning in text:
        return text
    separator = "\n\n" if text.strip() else ""
    return f"{text}{separator}{warning}"


def _append_factuality_warning_response(response_data: Dict) -> bool:
    """Append factuality warning to assistant content without mutating tool-call params."""
    if not isinstance(response_data, dict):
        return False
    ...
    if isinstance(content, str):
        message["content"] = _append_warning_to_text(content, FACTUALITY_WARNING_RESPONSE)
        return True
    ...
    if isinstance(content, list):
        ...
        for part in content:
            if not isinstance(part, dict):
                continue
            text = part.get("text")
            if isinstance(text, str) and FACTUALITY_WARNING_RESPONSE in text:
                has_warning = True
                break

        if not has_warning:
            content.append({"type": "text", "text": FACTUALITY_WARNING_RESPONSE})
        return True

    if isinstance(content, dict):
        text = content.get("text")
        if isinstance(text, str):
            content["text"] = _append_warning_to_text(text, FACTUALITY_WARNING_RESPONSE)
            return True

    return False


def _conversation_until_latest_user(messages: List[Dict]) -> List[Dict]:
    """Return conversation turns truncated at the latest user message (inclusive)."""
...
async def _invoke_guardrail_factcheck(
    messages: List[Dict],
    response_data: Dict,
) -> Dict[str, Any]:
    """Run filter+factcheck endpoint and return response metadata."""
    assistant_text = _extract_assistant_text_from_openai_response(response_data)
    tool_arguments = _extract_tool_arguments_from_openai_response(response_data)

    if not assistant_text and not tool_arguments:
        return {
            "ok": False,
            "error": "missing_input",
            "assistant_present": bool(assistant_text),
            "tool_arguments_present": bool(tool_arguments),
        }

    payload = {
        "conversation_turns": _conversation_until_latest_user(messages),
        "generated_text": assistant_text,
        "generated_tool_arguments": tool_arguments,
    }

    endpoint = f"{config.guardrail_base_url.rstrip('/')}/guardrail/filter_run"

    try:
        async with httpx.AsyncClient(timeout=60.0, trust_env=True) as guardrail_client:
            response = await guardrail_client.post(endpoint, json=payload)
    except Exception as error:
        return {
            "ok": False,
...
async def lifespan(app):
    ...
    print(f"Factuality Guardrail: {'Enabled' if FACTUALITY_GUARDRAIL_ENABLED else 'Disabled'}")
...
async def _proxy_openai_response(
    ...
    if response.status_code < 400 and CONTENT_GUARDRAIL_ENABLED:
        ...
        assistant_text = _extract_assistant_text_from_openai_response(response_data)
        tool_arguments = _extract_tool_arguments_from_openai_response(response_data)
        moderation_input_text = _build_moderation_input_text(assistant_text, tool_arguments)

        moderation_result = await _invoke_guardrail_moderation(
            latest_user_text=latest_user_text,
            assistant_text=moderation_input_text,
            request_id=request_id,
        )
        ...
        if isinstance(verdict, str) and verdict.lower() != "safe":
            ...
            print(
                ...
                f"    assistant_present={bool(moderation_input_text)} preview={_truncate_for_log(moderation_input_text) if moderation_input_text else '[empty]'}\n"
                f"    guardrail_response_preview={debug_body_preview}"
            )
    elif response.status_code < 400:
        print(f"  [{request_id}] Content guardrail disabled; skipping moderation")

    if response.status_code < 400 and FACTUALITY_GUARDRAIL_ENABLED and not moderation_blocked:
        tool_arguments = _extract_tool_arguments_from_openai_response(response_data)
        factcheck_result = await _invoke_guardrail_factcheck(
            messages=body.get("messages", []),
            response_data=response_data,
        )

        factuality_failed = False
        factuality = None

        if not factcheck_result.get("ok"):
            factuality_failed = True
            print(
                f"  [{request_id}] Factcheck API failed "
                f"(reason={factcheck_result.get('error')}, status={factcheck_result.get('status_code')}); "
                "treating as non-factual"
            )
        else:
            factcheck_payload = factcheck_result.get("body")
            factcheck_data = factcheck_payload.get("factcheck") if isinstance(factcheck_payload, dict) else None
            factuality = factcheck_data.get("factuality") if isinstance(factcheck_data, dict) else None

            if not isinstance(factuality, (int, float)):
                factuality_failed = True
                print(
                    f"  [{request_id}] Factcheck factuality unavailable ({factuality}); "
                    "treating as non-factual"
                )
            elif float(factuality) < FACTUALITY_MIN_SCORE:
                factuality_failed = True
                print(
                    f"  [{request_id}] Factcheck factuality={factuality} < {FACTUALITY_MIN_SCORE}; "
                    "non-factual"
                )

        if factuality_failed:
            model_name = body.get("model", response_data.get("model", "unknown"))
            if tool_arguments:
                print(
                    f"  [{request_id}] Tool-call params failed factuality; "
                    "returning FACTUALITY_ERROR_TOOL_CALL without tool calls"
                )
                response_data = _build_text_openai_response(
                    model=model_name,
                    text=FACTUALITY_ERROR_TOOL_CALL,
                )
                response.status_code = 200
            else:
                warning_added = _append_factuality_warning_response(response_data)
                if warning_added:
                    print(f"  [{request_id}] Appended FACTUALITY_WARNING_RESPONSE to response")
                else:
                    response_data = _build_text_openai_response(
                        model=model_name,
                        text=FACTUALITY_WARNING_RESPONSE,
                    )
                    response.status_code = 200
                    print(
                        f"  [{request_id}] Response had no appendable text content; "
                        "returned FACTUALITY_WARNING_RESPONSE as plain text"
                    )
        else:
            print(f"  [{request_id}] Factcheck factuality={factuality}; allowing response")
    elif response.status_code < 400 and not FACTUALITY_GUARDRAIL_ENABLED:
        print(f"  [{request_id}] Factuality guardrail disabled; skipping factcheck")
