#!/bin/bash
# inject_openclaw.sh - Send OpenClaw package + API key to enclave via vsock
#
# This script runs on the PARENT EC2 INSTANCE (not inside enclave)
# It downloads OpenClaw from npm and injects it into the enclave via vsock.
#
# Usage:
#   ./inject_openclaw.sh [version] [enclave-cid] [openai-api-key] [gateway-token] [openrouter-api-key] [serper-api-key]
#
# Example:
#   ./inject_openclaw.sh 1.2.3 16 sk-proj-YOUR-OPENAI-KEY your-gateway-token sk-or-YOUR-OPENROUTER-KEY YOUR-SERPER-KEY
#
# Note: API keys and gateway token are injected at runtime, so they don't affect PCR2 measurement
...
WORK_DIR="/tmp/openclaw_injection_$$"
CACHE_DIR="$HOME/.cache/openclaw_injection"
TARBALL_PATH="$WORK_DIR/openclaw.tgz"

echo "=========================================="
echo "  OpenClaw Injection to Enclave"
echo "=========================================="
echo ""
echo "Configuration:"
...
echo "  OPENAI_API_KEY:     ${OPENAI_API_KEY:+provided}"
echo "  OPENROUTER_API_KEY: ${OPENROUTER_API_KEY:+provided}"
echo "  SERPER_API_KEY:     ${SERPER_API_KEY:+provided}"
echo "  Gateway Token:   ${GATEWAY_TOKEN:+provided}"

if [ -z "$OPENAI_API_KEY" ]; then
    echo ""
    echo "⚠ WARNING: No OPENAI_API_KEY provided!"
    echo "  OpenClaw will be configured but may not work without an OpenAI key."
    echo "  Usage: $0 <version> <cid> <openai-api-key> <gateway-token> [openrouter-api-key] [serper-api-key]"
    echo ""
fi
...
# ============================================================================
# Step 2: Download OpenClaw from npm (or use cache)
# ============================================================================

echo "[2/4] Preparing OpenClaw v${OPENCLAW_VERSION}..."
...
if [ "$OPENCLAW_VERSION" = "latest" ]; then
    ...
    echo "  Latest version: $OPENCLAW_VERSION"
fi

# Check cache first
mkdir -p "$CACHE_DIR"
CACHED_TARBALL="$CACHE_DIR/openclaw-${OPENCLAW_VERSION}.tgz"
...
if [ -f "$CACHED_TARBALL" ]; then
    ...
    # Install openclaw locally with all dependencies
    mkdir -p package_bundle
    cd package_bundle

    if [ "$OPENCLAW_VERSION" = "latest" ]; then
        npm install openclaw --production --no-save
    else
        npm install openclaw@${OPENCLAW_VERSION} --production --no-save
    fi

    # Extract actual version from node_modules (in case version was "latest")
    ACTUAL_VERSION=$(node -p "require('./node_modules/openclaw/package.json').version")
...
# Create metadata JSON (including API keys and gateway token if provided)
METADATA="{"
METADATA="$METADATA\"package\": \"openclaw\","
METADATA="$METADATA\"version\": \"$OPENCLAW_VERSION\","
METADATA="$METADATA\"sha256\": \"$HASH\","
METADATA="$METADATA\"size_bytes\": $TARBALL_SIZE"
if [ -n "$OPENAI_API_KEY" ]; then
    METADATA="$METADATA,\"openai_api_key\": \"$OPENAI_API_KEY\""
    METADATA="$METADATA,\"api_key\": \"$OPENAI_API_KEY\""
fi
if [ -n "$OPENROUTER_API_KEY" ]; then
    METADATA="$METADATA,\"openrouter_api_key\": \"$OPENROUTER_API_KEY\""
fi
if [ -n "$SERPER_API_KEY" ]; then
    METADATA="$METADATA,\"serper_api_key\": \"$SERPER_API_KEY\""
fi
...
echo "  1. Installing OpenClaw v${OPENCLAW_VERSION}"
echo "  2. Configuring it to use the guardrail proxy"
echo "  3. Starting the OpenClaw gateway"
echo ""
...
echo ""
echo "Cache info:"
echo "  Cached tarball: $CACHE_DIR/openclaw-${OPENCLAW_VERSION}.tgz"
